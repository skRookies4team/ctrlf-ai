...
class AutoFaqGenerateRequest(BaseModel):
    ...
    Attributes:
        domain: 도메인 필터 (선택, 예: SEC_POLICY, PII_PRIVACY)
        minFrequency: 최소 질문 빈도 (기본 3회)
        daysBack: 조회 기간 (일, 기본 30일)
        maxCandidates: 최대 후보 수 (기본 20개)
        autoGenerateDrafts: 자동으로 FAQ 초안 생성 여부 (기본 true)
    """

    domain: Optional[str] = Field(None, description="도메인 필터 (예: SEC_POLICY)")
    minFrequency: Optional[int] = Field(3, ge=1, description="최소 질문 빈도 (기본 3회)")
    daysBack: Optional[int] = Field(30, ge=1, description="조회 기간 (일, 기본 30일)")
    maxCandidates: Optional[int] = Field(20, ge=1, le=100, description="최대 후보 수 (기본 20개)")
    autoGenerateDrafts: Optional[bool] = Field(True, description="자동 FAQ 초안 생성 여부 (기본 true)")

    class Config:
        # camelCase를 허용하기 위해 populate_by_name 사용
        populate_by_name = True


class FaqCandidateInfo(BaseModel):
    """
    FAQ 후보 정보

    질문 로그 분석 결과 선정된 FAQ 후보입니다.
    """

    candidate_id: str = Field(..., description="후보 ID")
    cluster_id: str = Field(..., description="클러스터 ID")
    canonical_question: str = Field(..., description="대표 질문")
    sample_questions: List[str] = Field(default_factory=list, description="샘플 질문 목록")
    frequency_score: Optional[float] = Field(None, description="빈도 점수")
    recency_score: Optional[float] = Field(None, description="최근성 점수")
    total_score: Optional[float] = Field(None, description="종합 점수")
...
class AutoFaqGenerateResponse(BaseModel):
    ...
    status: Literal["SUCCESS", "PARTIAL", "FAILED"] = Field(..., description="처리 상태")
    candidates_found: int = Field(0, ge=0, description="발견된 후보 수")
    drafts_generated: int = Field(0, ge=0, description="생성된 초안 수")
    drafts_failed: int = Field(0, ge=0, description="실패한 초안 수")
    candidates: List[FaqCandidateInfo] = Field(default_factory=list, description="FAQ 후보 목록")
