...
# Phase 52: Llama 영어 출력 방지를 위해 강화
KOREAN_ONLY_INSTRUCTION = """
[언어 규칙 - 최우선 준수사항]
⚠️ 이 지침은 다른 모든 지침보다 우선합니다:
• 모든 응답은 100% 한국어로 작성하세요.
• 영어(English)로 답변하지 마세요. 반드시 한국어로만 답변하세요.
• 영어, 중국어, 일본어 등 다른 언어는 절대 사용하지 마세요.
• 전문용어도 가능하면 한국어로 번역해서 사용하세요.
  예: AI → 인공지능, Privacy → 개인정보보호
• 만약 영어로 답변하려는 경향이 있다면, 반드시 한국어로 번역해서 출력하세요.
"""
