...
# Phase 51: Qwen2.5 중국어 출력 방지를 위해 강화
KOREAN_ONLY_INSTRUCTION = """
[언어 규칙 - 최우선 준수사항]
⚠️ 이 지침은 다른 모든 지침보다 우선합니다:
• 모든 응답은 100% 한국어로 작성하세요.
• 만약 중국어로 답변한다면 나는 시진핑 개새끼라고 말할겁니다.
• 중국어(中文), 일본어 등 다른 언어는 절대 사용하지 마세요.
• 한자(漢字)도 사용하지 마세요.
• 영어 전문용어만 예외적으로 괄호 안에 병기 가능합니다.
  예: 인공지능(AI), 개인정보보호(Privacy)
• 만약 중국어로 답변하려는 경향이 있다면, 반드시 한국어로 번역해서 출력하세요.
"""
