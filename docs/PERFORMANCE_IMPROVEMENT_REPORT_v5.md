...
#### 6.4.4 Edge Case 분석: "복무 규정" 0건 원인

"복무 규정" 쿼리가 sources=0을 반환하는 원인을 파이프라인 단계별로 추적했습니다.

**Step 1: Anchor Keyword 추출**
```
Query: "복무 규정 알려줘"
Extracted: {"복무"} (규정/알려줘는 불용어로 제거)
```

**Step 2: Milvus 검색 결과**
| 순위 | Score | 실제 내용 |
|------|-------|----------|
| 1 | 0.595 | 민방위대/예비군 목적 조항 |
| 2 | 0.586 | 예비군 피복 대장관리 |
| 3 | 0.546 | 민방위대원 복장 규정 |

**Step 3: Gate 판정**
```
Score Gate:  max_score=0.595 < threshold=0.60 → DEMOTED ❌
Anchor Gate: "복무" in sources? → False → DEMOTED ❌ (2중 안전장치)
```

**분석 결과**
| 항목 | 상세 |
|------|------|
| 사용자 의도 | 복무 규정 (근무시간, 휴가, 출퇴근 등) |
| 실제 검색 결과 | 민방위대/예비군 복장 규정 |
| max_score | 0.595 (threshold 0.60 미달) |
| "복무" 포함 여부 | 검색 결과에 없음 |

**결론**
```
✅ Low-relevance Gate 정상 작동

1. KB(사규.docx)에 실제 "복무 규정" 내용이 없음
2. Milvus가 유사 단어("복장")가 있는 민방위/예비군 규정을 반환
3. Score Gate: 저관련 결과(0.595 < 0.60) 감지 → 강등
4. Anchor Gate: "복무" 키워드 미매칭 감지 → 2중 강등

→ 임계값 조정 불필요 (Gate가 올바르게 작동 중)
→ 해결책: KB에 실제 "복무 규정" 문서 추가 필요
```
