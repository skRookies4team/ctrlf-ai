...
```python
INFRA_BASE_URL: str = ""  # 인프라 서비스 URL
```
...
| `803d039` | S3 presigned URL 요청을 infra-service로 변경, INFRA_BASE_URL 설정 추가 |
...
| 환경변수 | 설명 | 예시 |
|----------|------|------|
| `INFRA_BASE_URL` | 인프라 서비스 URL | `http://infra-service:8080` |
...
# 신규 추가 (12/24)
INFRA_BASE_URL=http://infra-service:8080
...
1. `INFRA_BASE_URL` 설정 시 trailing slash 없이 설정
...
| 파일 | 변경 내용 |
|------|----------|
| `app/core/config.py` | INFRA_BASE_URL, EMBEDDING_BASE_URL 추가 |
